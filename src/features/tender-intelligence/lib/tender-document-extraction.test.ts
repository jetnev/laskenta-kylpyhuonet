...
describe('tender-document-extraction helpers', () => {
  it('recognizes supported mime types and maps them to extractor types', () => {
    ...
    expect(getTenderDocumentExtractionSupport('application/pdf').extractorType).toBe('pdf');
    expect(getTenderDocumentExtractionSupport('application/vnd.openxmlformats-officedocument.wordprocessingml.document').extractorType).toBe('docx');
